...
with open("energy_forecast_model.onnx", "wb") as f:
    f.write(onnx_model.SerializeToString())
print("Model successfully exported to energy_forecast_model.onnx")

# Test the ONNX model
sess = rt.InferenceSession("energy_forecast_model.onnx", providers=["CPUExecutionProvider"])
input_name = sess.get_inputs()[0].name
preds = sess.run(None, {input_name: X_test.to_numpy().astype(np.float32)})[0]
print("ONNX test run successful, first 5 predictions:", preds[:5])
